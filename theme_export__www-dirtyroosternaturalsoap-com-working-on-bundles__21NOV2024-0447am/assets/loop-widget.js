...
const CREATE_BUNDLE_TRANSACTION_API =
  "https://api-service.loopwork.co/bundleTransaction";
const FILTER_BUNDLE_SP_API =
  "https://apiv2.loopwork.co/bundleStorefront/filterBundleSellingPlanIds";
const GET_PRODUCT_BUNDLE_DATA_API =
  "https://apiv2.loopwork.co/bundleStorefront/getChildVariants?id=";
...
let customSettings = {
  ...
  translationData: {
    ...
    widget_add_to_cart_button_for_subscription: "Add subscription to cart",
  },
  byDefaultChooseSubscriptionOption: true,
...
function showLoopPurchaseOptionsLabel(productId) {
  ...
  if (elements) {
    for (const e of elements) {
      e.classList.remove("loop-display-none", "loop-display-none-by-variant");
    }
  }
}
...
function observeFormChangesLoop(productData) {
  document.querySelectorAll("form").forEach((form) => {
    ...
    const loopVariantElement = form.querySelector('[name="loop_variant_id"]');
...
function defaultSelectFirstSellingPlanLoop(variant, productId) {
  const loopPurchaseOptionsContainer = getLoopSubscriptionContainer(productId);
  ...
  loopPurchaseOptions.forEach((element) => {
    if (
      ...
      spgIds.includes(window.loopProps[productId].previousSPGId)
        ? element.dataset.id == window.loopProps[productId].previousSPGId
        : isFirstOption)
    ) {
      isFirstOption = false;
...
function hideLoopPurchaseOptionsLabel(productId) {
  ...
  if (elements) {
    for (const e of elements) {
      e.classList.add("loop-display-none", "loop-display-none-by-variant");
    }
  }
}
...
function displayLoopTooltip(productId) {
  ...
  const description = loopContainer.querySelector("#loop-tooltip-description");
...
function hideLoopTooltip(productId) {
  const element =
    getLoopSubscriptionContainer(productId).querySelector("#loop-tooltip");
  if (element) {
    element.classList.add("loop-display-none");
  }
}
...
function applyLoopSettings({ productId }) {
  ...
  if (
    ...
    const { availableSellingPlanAllocations = [] } =
      window.loopProps[productId];
    let ids = [];
  ...
  if (
    ...
    let parentElement =
      getLoopSubscriptionContainer(productId).querySelector(parentId);
...
function toggleVariantDisplayLoop(product, selectedVariantId) {
  product.variants.forEach((variant) => {
    const displayStyle = variant.id === selectedVariantId ? "block" : "none";
...
function changeInSellingPlanGroupLoopMobile(
  ...
  let elements =
    getLoopSubscriptionContainer(productId).querySelectorAll(removeElementId);
  if (elements) {
    elements.forEach((element) => {
      if (element) {
  ...
  if (sellingPlanGroupId === "loop-one-time-purchase") {
    ...
    let element =
      getLoopSubscriptionContainer(productId).querySelector(elementId);
    if (element) {
      element.classList.add("loop-selected-selling-plan-group");
    }
  }
  ...
  if (window && window.loopProps && window.loopPropsUI.displayDiscountBadge) {
    selling_plan_groups.map((spg) => {
      ...
      selling_plans.map((sp) => {
        const delFreq = prepaidPlansData[sp.id]?.deliveriesPerBillingCycle || 1;
        ...
        discountList.push({
          ...
          amount:
            priceAdj.value_type === "fixed_amount"
              ...
              ? Number(variant.price) - Number(priceAdj.value / delFreq)
      ...
      if (deliveryFreq > 1) {
        ...
        let ssp =
          selling_plans.find(
            (sp) => sp.id === Number(variantFirstSellingPlan.selling_plan_id)
          ) || {};
        selectedDiscount = ssp.price_adjustments[0];
        deliveryFreq = prepaidPlansData[ssp.id]?.deliveriesPerBillingCycle || 1;
      }
      ...
      if (element) {
        ...
        if (
          (!selectedDiscount?.value && !Number(selectedDiscount?.value)) ||
          (selectedDiscount &&
            selectedDiscount.value_type === "price" &&
            Math.round(
              Number(variant.price) -
...
function calculateCurrentSellingPlanLoop({
  ...
  return (
    availableSellingPlanAllocations.find((sellingPlan) => {
      ...
      if (sellingPlanId) {
        return Number(sellingPlan.selling_plan_id) === Number(sellingPlanId);
      }
      return true;
    }) || {}
  );
...
function updateLoopProperties({
  ...
  let availableSellingPlanAllocations =
    variant && Array.isArray(variant.selling_plan_allocations)
      ? variant.selling_plan_allocations
      : [];
  window.loopProps[productId].availableSellingPlanAllocations =
    availableSellingPlanAllocations;
...
function updateCartButtonTextLoop({ productId }) {
  const parentElement =
    document.querySelector(`[data-loop-product-id="${productId}"]`) || document;
...
function determineOneTimeOrderLoop(productId) {
  ...
  return !sellingPlanGroupId || sellingPlanGroupId === "loop-one-time-purchase";
}
...
function loopFormatMoney(price, removeEach) {
  ...
  const moneyFormat = document.querySelector(
    "#loop-price-money-format"
  ).innerText;
...
function loopFormatPrice(price, moneyFormat, moneyWithoutCurrency) {
  ...
  if (moneyFormat.includes("0.00")) {
    ...
    return moneyFormat.replace("0,00", priceValue.toFixed(2).replace(".", ","));
...
function updateLoopSellingPlanDescriptionUI({ productId }) {
  ...
  updateLoopSellingPlanDescriptionElement(descriptionElement, descriptionValue);
}
...
function updatePriceInUILoop({ productId }) {
  ...
  selling_plan_groups.map((spg) => {
    if (Array.isArray(spg.selling_plans) && spg.selling_plans.length) {
      ...
      const { selling_plan_group_id, per_delivery_price } =
...
function getLoopSubscriptionContainer(productId) {
  return document.querySelector(`#${loopSubscriptionContainerId}-${productId}`);
}
...
async function getLoopBundleSpgs(productId) {
  const spgs =
    loopProps[productId]?.product?.selling_plan_groups?.flatMap((spg) => {
  ...
  try {
    const [bundleRes, prepaidRes] = await Promise.all([
      fetch(FILTER_BUNDLE_SP_API, {
        method: "POST",
    ...
    console.error("getLoopBundleSpgs", error);
  }
}

function hideLoopBundleSPG(productId) {
  if (
    !window.loopProps?.[productId]?.["productBundleData"]?.["isBundleProduct"]
  ) {
...
function clickUpdatedSPGLoop(productId) {
  ...
  const selectedVariant = getLoopVariantId(productId);
  ...
  const variantIdFromUrl =
    ...
    getFirstAvailableVariantVariantIdLoop(productId);
  const variant = variants.find((v) => v.id == variantIdFromUrl) ?? variants[0];
  ...
  try {
    ...
    if (bundleData) {
      ...
      if (bundleData.isBundleProduct) {
        ...
        overrideAddToCartButtonLoop(productId, bundleData.mapping);
    ...
    console.error("processLoopBundleProduct", error);
  }
}
...
function checkAllowCheckoutIfBundle(productId) {
  ...
  if (window.loopProps[productId]?.productBundleData?.isBundleProduct) {
    const selectedVariant =
      window.loopProps[productId].productBundleData.mapping[selectedVariantId];
...
function applyLoopBundleDiscount(productId) {
  ...
  selling_plan_groups.map((spg) => {
    if (Array.isArray(spg.selling_plans) && spg.selling_plans.length) {
      ...
      const { selling_plan_group_id, price, compare_at_price } =
      ...
      if (elementOriginal && element?.innerHTML == elementOriginal?.innerHTML) {
  ...
  if (loopOneTimePrice) {
    if (bundlePriceOnetimeDiscounted) {
      bundlePriceOnetime = bundlePriceOnetimeDiscounted * 100 * conversionRate;
    }
...
function overrideAddToCartButtonLoop(productId, bundleVariants) {
  ...
  productForms.forEach((form) => {
    ...
    submitButtons.forEach((btn) => {
      ...
      btn.addEventListener("click", (event) => {
        const clickedProductId = form.getAttribute("data-loop-product-id");
        const quantity = document.querySelector(PRODUCT_QUANTITY_SELECTOR);
        loopHandleAddToCart(
          event,
          clickedProductId,
          quantity?.value,
...
async function applyShopifyDiscountLoop(discountCodes) {
  try {
...
function createBundleTransactionPayload(
  ...
  return {
    payload: {
      ...
      bundleVariantId: bundleData?.productVariantMapping?.[selectedVariant],
      bundleDiscountId: bundleDiscountId,
      sellingPlanId:
        bundleData?.productVariantSellingPlanMapping?.[selectedSellingPlan],
...
async function handleBundleTransaction(
  ...
  try {
    const { payload, bundleVariantDiscount } = createBundleTransactionPayload(
      productId,
      quantity,
      selectedSellingPlan
    );
...
function getSelectedSellingPlanLoop(productId) {
  ...
  document
    .querySelectorAll(`form[data-loop-product-id="${productId}"]`)
    .forEach((form) => {
      const existingInput = form.querySelector('input[name="selling_plan"]');
...
async function getBundleDiscountAttributesLoop() {
  ...
  const bundleIdsInCart = new Set(
    res.items.map((item) => item.properties?._bundleId).filter((id) => id)
  );

  const filteredAttributes = Object.keys(loopBundleDiscountAttributes)
...
async function createAddToCartPayloadLoop(
  bundleTransactionId,
  bundleVariantDiscount,
  selectedSellingPlan,
  ...
  const oldAttr = await getBundleDiscountAttributesLoop();
  ...
  currentDiscountAttribute[bundleTransactionId] = {
    ...
    discountComputedValue: bundleVariantDiscount
      ? selectedSellingPlan
        ? bundleVariantDiscount.spbpDiscount[selectedSellingPlan] * quantity
  ...
  if (selectedBundleVariantProducts?.length) {
    selectedBundleVariantProducts.forEach((product) => {
      ...
      if (bundleData.showBundleName) {
        obj.properties.bundleName = bundleData?.loopBundleName ?? "";
      } else {
        obj.properties._bundleName = bundleData?.loopBundleName ?? "";
      }
...
async function loopHandleAddToCart(
  event,
  productId,
  quantity = 1,
  bundleVariants
  ...
  const selectedBundleVariantId = window.loopProps[productId].selectedVariantId;
  ...
  let selectedSellingPlan = getSelectedSellingPlanLoop(productId);
  ...
  const payload = await createAddToCartPayloadLoop(
    bundleTransactionId,
    bundleVariantDiscount,
    selectedSellingPlan,
    selectedBundleVariantId,
...
async function dispatchLoopAddCartEvent(
  ...
  // const res = await applyShopifyDiscountLoop(commaSeparatedDiscountString);
  const addToCartEvent = new CustomEvent("loopPresetAddToCartSuccessEvent", {
    detail: {
      productId: productId,
      response: response,
    },
  ...
  selling_plan_groups.map((spg) => {
    if (Array.isArray(spg.selling_plans) && spg.selling_plans.length) {
      ...
      const deliveriesPerBillingCycle =
        prepaidSellingPlans[firstSellingPlan.id]?.deliveriesPerBillingCycle ||
        1;
      let sellingPlanAllcotion =
        selling_plan_allocations.find(
          (a) => Number(a.selling_plan_id) === Number(firstSellingPlan.id)
...
function hideDifferentVariantSellingPlansLoop(productData) {
  productData.variants.forEach((variant) => {
    var sellingPlanIds = variant.selling_plan_allocations.map((allocation) =>
      allocation.selling_plan_id.toString()
    );
